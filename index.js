...
// Volvemos al formato HTTP porque Google Cloud Run te envía peticiones POST
exports.notificarNuevoLead = async (req, res) => {
  try {
    console.log("🔔 Petición HTTP recibida. Buscando leads pendientes...");

    // 1. Buscamos TODOS los leads que tengan status 'nuevo'
    const leadsRef = db.collection('leads_corporativos');
    const snapshot = await leadsRef.where('status', '==', 'nuevo').get();

    if (snapshot.empty) {
      console.log("⚠️ No hay leads nuevos pendientes de notificación.");
      return res.status(200).send("No hay leads pendientes");
    }
    ...
    for (const doc of snapshot.docs) {
      ...
      console.log(`📧 Correo enviado para el lead: ${data.email}`);

      // 4. Marcar este lead específicamente como procesado
      await doc.ref.update({ 
        emailSent: true,
        status: 'notificado' 
      });
      console.log(`📝 Documento de ${data.email} actualizado en Firestore.`);
    }

    // 5. Respondemos a Cloud Run que todo salió bien
    return res.status(200).send(`Se procesaron ${snapshot.size} leads exitosamente.`);
